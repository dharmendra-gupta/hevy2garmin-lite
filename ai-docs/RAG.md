...
| POST | `/v1/mappings/learn-from-garmin/{hevy_workout_id}?include_mapped=false` | Basic | "Learn from Garmin" — reads back a manually-corrected exercise (category **and** name); 404 if no synced Garmin activity for that workout; 502 if the Garmin session died mid-request (resets the cached client). `include_mapped` (default `false`, dashboard checkbox, added 2026-08-17) widens the scan to also re-check template_ids that already have a specific mapping — for when the existing mapping is confidently wrong, not just missing |
