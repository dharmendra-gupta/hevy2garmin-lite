...
### Same-day follow-up: "Map from Garmin" was still broken after fix #3, for two further reasons

Deploying fix #3 above didn't actually fix "Map from Garmin" — the user tried it against a real workout and still got "no changes found." Live debugging (real, read-only Hevy/Garmin API calls against the account, with the user's explicit go-ahead) found two more real bugs stacked on top of each other:

4. **`src/learn.py` had its own second, redundant gate** — `if template_id in TEMPLATE_TO_FIT or template_id in already_known_template_ids: continue` — independent of whatever the fixed `known_template_ids()` computed. The bare `TEMPLATE_TO_FIT` membership check alone still blocked every catalog-present-but-fallback-resolved exercise. Fixed by removing that redundant clause.
5. **`main.py`'s route never passed the live dashboard-configured timeline-tuning values** (`working_set_seconds` etc.) into `learn_mappings_from_garmin()` — it always reconstructed the timeline with the hardcoded `40/25/75/120` defaults, even though the account's real live settings were `45/20/80/130`. Since the join was exact-string `startTime` matching, this silently broke it for any account that had ever touched the Timeline Tuning panel (which this project made live-editable earlier the same day).

Fixing #5 alone still didn't produce an exact match for the specific workout under test, because the timeline-tuning values had likely changed *between* when that workout was originally pushed and when "Map from Garmin" was retried — there's no record of what config was in effect at push time, so no config tried at learn-time could reproduce it exactly. This exposed the deeper structural fragility: **the entire `startTime`-reconstruction approach was the wrong design**, not just the two bugs above.

**Redesigned same day**: the user proposed matching by array position / per-exercise set count instead of reconstructing time at all — Hevy's workout already carries an exact set-count-per-exercise (`[2, 2, 3, 3, 2, 3]` for the diagnosed workout), and Garmin's total ACTIVE-set count on a synced activity should equal the same total. Verified live before implementing (same discipline as the webhook-payload fix): fetched the raw `GET exerciseSets` response for the real corrected workout and confirmed **array position is preserved exactly, even around the manually-corrected entry** — only `wktStepIndex`/`messageIndex` null out, not the entry's place in the array. `learn_mappings_from_garmin()` was rewritten to split ACTIVE-only entries into consecutive groups sized by Hevy's own per-exercise set counts, with two hard safety bail-outs (learn nothing rather than guess): total ACTIVE count must equal total Hevy set count, and a group's entries must all agree on `(category, name)`. Verified end-to-end against the real workout before shipping — it now correctly learns the exact correction the user made.

**A third, unrelated bug surfaced during this same live verification**: the grouped identity for the corrected exercise came back as `category="BANDED_EXERCISES"`, which `_validate_category_name_pair()` rejected — not because it's invalid, but because `mapping.py`'s `CATEGORY_NAMES` table was a hand-typed subset that stopped at category id 32, silently missing 19 real categories (ids 33-53) that `fit_tool`'s own `ExerciseCategory` enum has always had. This is exactly why the 8 category ids previously documented as "unresolved, falls back to TOTAL_BODY" behaved that way — never a genuine Garmin limitation. Fixed by building `CATEGORY_NAMES` directly from the real enum instead of hand-typing it.

### Re-learn already-mapped exercises from Garmin (2026-08-17)

A second real-usage report the same day: "Map from Garmin" only ever offers to learn a template_id with *no* specific mapping yet. But a bundled-catalog mapping can be specific *and confidently wrong* — a real example ("Lateral Dumbbell Raise" rendering as "Leaned Lateral Raise" on Garmin) was just as permanently stuck as an unmapped exercise, since `known_template_ids()` counted it as done either way. See `ai-docs/relearn_mapped_exercises_plan.md` for the full plan. Implemented as a pure caller-side change — `learn_mappings_from_garmin()`'s core logic and test coverage are untouched, since it already just takes an `already_known_template_ids` set as a plain parameter. `main.py`'s route gained an opt-in `include_mapped` query param (default `false`) that passes `set()` instead of `mapper.known_template_ids()` when on, surfaced via a checkbox in the dashboard's Sync History panel. The route also now logs the old → new identity when it overwrites a previously-known mapping, closing the observability gap that made the original bug hard to notice in the first place.
