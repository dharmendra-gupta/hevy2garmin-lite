...
**Webhook — the fast path.** Hevy supports `POST /v1/webhook-subscription` (`url`, `auth_token`), firing `{"event": "workout.created", "workout": {...}}` to our `POST /v1/webhooks/hevy` on every new workout, with a 5-second response budget. (Endpoint shapes corroborated across several independent third-party integrations, not Hevy's own official docs — worth double-checking against the first real payload.)
